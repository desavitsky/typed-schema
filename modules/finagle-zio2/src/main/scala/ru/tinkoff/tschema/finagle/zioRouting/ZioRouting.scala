...
trait ZioRoutingInstances extends ZioRoutingInstances2 { self: ZioRouting.type =>
  ...
  final implicit def zioRunnableU[R: Tag](implicit
  ...
  implicit def ziosRunnableU[R: Tag](
      implicit // there was R <: Has[]
...
trait ZioRoutingInstances2 {
  ...
  implicit def zioRunnable[R: Tag, E <: Throwable](implicit
  ...
  implicit def ziosRunnable[R: Tag, E <: Throwable](
      implicit // < Has
  ...
  private[this] def execs[R: Tag, E <: Throwable]( // Has
    ...
    execResponse[R, R with ZRouting, E](
      ...
      env => env ++ ZEnvironment(ZRouting(request, SubString(request.path), 0))
    )

  private[this] def exec[R: Tag, E <: Throwable](
      runtime: zio.Runtime[R],
    ...
    execResponse[R, ZioRouting[R], E](
      runtime,
      catchRejections(zioResponse),
      env => ZEnvironment(ZioRouting(request, SubString(request.path), 0, env.get))
    )
  }
