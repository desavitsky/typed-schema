...
import ru.tinkoff.tschema.finagle.zioRouting.{Fail, ZIOHttp, ZRouting}
...
private[finagle] class ZioRoutedInstance[R: Tag, E] extends RoutedPlus[ZIOHttp[R, E, *]] {
  ...
  def withMatched[A](m: Int, fa: F[A]): F[A] = fa.provideSomeEnvironment(env => ZEnvironment(env.get.copy(matched = m)))
